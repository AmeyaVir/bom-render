...
class WorkflowService:
    ...
    def start_workflow(self, workflow_id, wi_document, item_master=None, comparison_mode='full'):
        ...
        """
        try:
            wi_path = self.file_storage.save_file(wi_document, workflow_id)
            
            item_path = None
            if item_master:
                item_path = self.file_storage.save_file(item_master, workflow_id)
    ...
    def _save_workflow_results(self, workflow_id, results, summary):
        results_file = self.file_storage.save_results(workflow_id, {'matches': results, 'summary': summary})
    ...
    def get_workflow_status(self, workflow_id):
        ...
        if workflow['status'] == 'completed' and not workflow['has_results']:
            try:
                # Check for results file existence
                self.file_storage.get_results(workflow_id)
                workflow['has_results'] = True
            except FileNotFoundError:
                workflow['has_results'] = False
        
        return workflow
    
    def get_workflow_results(self, workflow_id):
        return self.file_storage.get_results(workflow_id)
    ...
    def get_all_workflows(self):
        ...
        for workflow in workflows:
            try:
                self.file_storage.get_results(workflow['id'])
                workflow['has_results'] = True
            except FileNotFoundError:
                workflow['has_results'] = False
